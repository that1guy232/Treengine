...
class Renderable:
    ...
    def copy(self, x, y):
        return self.__class__(
            x, y, self.width, self.height, self.color, self.line_width
        )
...
class Box(Renderable):
    ...
    def __init__(
        self, x, y, width, height, color=(255, 255, 255), line_width=0, visible=True
    ):
...
class Circle(Renderable):
    ...
    def draw(self, display, offset, zoom):
        ...
        # draw the circle
        pygame.draw.circle(
            display, self.color, (x, y), self.radius * zoom, self.line_width
        )
...
class Polygon(Renderable):
    ...
    def draw(self, display, offset, zoom):
        ...
        pygame.draw.polygon(
            display,
            (255, 255, 255),
            [(x + point.x * zoom, y + point.y * zoom) for point in self.points],
            1,
        )
...
class Texture(Box):
    ...
    def draw(self, display, offset, zoom):
        # if we are not visible, don't draw
        if not self.is_visible:
            return
        ...
        display.blit(
            self.scaled_texture, (x, y), (0, 0, self.width * zoom, self.height * zoom)
        )
